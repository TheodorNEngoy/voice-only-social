...
export async function POST(_: Request, { params }: { params: { handle: string } }) {
  ...
  await prisma.block.upsert({
    where: { blocker_id_blocked_id: { blocker_id: session.user.id, blocked_id: target.id } },
    update: {},
    create: { blocker_id: session.user.id, blocked_id: target.id }
  });
...
export async function DELETE(_: Request, { params }: { params: { handle: string } }) {
  ...
  const target = await resolveUser(params.handle);
  if (!target) return NextResponse.json({ error: "not_found" }, { status: 404 });
  await prisma.block.delete({ where: { blocker_id_blocked_id: { blocker_id: session.user.id, blocked_id: target.id } } });
  return NextResponse.json({ ok: true });
}
